...
/* 形参带 volatile：调用方常把 PID 放在 volatile 结构体里（chassis / gimbal），
   不写限定符等于在传参时把它丢掉，编译器会逐条报 -Wincompatible-pointer-types-
   discards-qualifiers。加限定符对非 volatile 的调用点同样合法，不影响任何现有调用。 */
void PID_Init(volatile PID *pid,float p,float i,float d,float maxSum,float maxOut);
void PID_SingleCalc(volatile PID *pid,float reference,float feedback);
void PID_CascadeCalc(CascadePID *pid,float angleRef,float angleFdb,float speedFdb);
void PID_Clear(volatile PID *pid);
